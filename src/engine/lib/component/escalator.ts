...
export default class Escalator extends Component {
  ...
  constructor(position: Vector, escalatorNumber: number = 6) {
    super(position);
    this.start = new Wheel(position, registry.engine.GameBoard.cellSize / 2, 'red');
  ...
  addComponent() {
    const wheel = new Wheel(this.centroid, registry.engine.GameBoard.cellSize / 2, 'red');
